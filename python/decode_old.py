...
def readCode(img,th=127):
    ...
    if len(codes) >= 1:
        barcode_info = codes[0].data.decode('utf-8')
        return barcode_info
    return None
...
def getProperties(inputStr):
    ...
    for i in range(0,len(inputArr)):
        if i == 0:
            framerate = inputArr[0]
        elif i == 1:
            packetsCount = inputArr[1]
        elif i == 2:
            extension = inputArr[2]
    return [framerate,packetsCount,extension] #empty json
...
def recover(image, droppedArray, interval, fileCount, lastResort):
    ...
    for index in range(startIndex, len(droppedArray)):
        ...
        for i in range(0,256):
            ...
            if lastResort or (i % 4 == 0 and i > 128 and i < 192):
            ...
            #for i in range(0,256):
                #print(i)
                #for x in range(1,10):
                    #if readCode(resizeBW(image,x * 10, i)) is not None:
                     #return readCode(resizeBW(image,x * 10, i))
                ...
                if code is not None:
                    ...
                    if code != "{vidqr}" and not code.startswith("{"):
                        ...
                        if packetID == fileCount + 1: #packetID is correct
                            ...
                            debug("MISSED: packet " + str(fileCount + 1))
                            itemMissed = True
