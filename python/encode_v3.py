...
def generateCodesFromArray(dataArray, typeID):
    ...
    for dataIndex in range(0,len(dataArray)):
        ...
        #Create 3-byte (24-bit int) header for packet ID and encode to Base64
        header = binascii.b2a_base64((dataIndex + 1).to_bytes(3, byteorder = 'big'))[:-1]
        typeHeader = binascii.b2a_base64(typeID.to_bytes(1, byteorder = 'big'))[:-1]

        #Generate QR code with header and file data
        img = generateQR(header + typeHeader + dataArray[dataIndex])
...
def encode(inputPath,outputPath):
    global flashes, versionOfCode, errorCorrection, framerate, extension, originalSize, permanentPrint, redundantFrames
    ...
    largeMetadata = binascii.b2a_base64(bytes('{"name":"' + fileName + '","ext":"' + extension + '","size":' + str(originalSize) + '}', 'utf-8'))
    ...
    images = generateCodesFromArray(splitValues, 0)

    splitMetadata = [largeMetadata[i: i + bytesPerCode] for i in range(0, len(largeMetadata), bytesPerCode)]
    metadataImages = generateCodesFromArray(splitMetadata, 1)
    ...
    metadata = str(version) + ":" + str(len(splitValues)) + ":" + str(len(splitMetadata))
    ...
    if len(images) > 0:
        ...
        for metaIndex in range(0, len(metadataImages)):
            #Write next image to video
            clearConsole()
            print("generating metadata...")
            out.write(includeMetadataBarcode(metadataImages[metaIndex],metadataBarcode))
