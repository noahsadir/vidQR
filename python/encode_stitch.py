...
def joinImages(imgArr, barCode,blank, interpolation=cv2.INTER_CUBIC):
    print(len(imgArr))
    resizedImages = []

    blank = cv2.resize(blank, (int(1600 / rows), int(1600 / rows)), interpolation=cv2.INTER_CUBIC)
    for i in range(0,codesPerPage):
        if len(imgArr) > i:
            img = imgArr[i]
            img = cv2.resize(img, (int(1600 / rows) ,int(1600 / rows)), interpolation=cv2.INTER_CUBIC)
            resizedImages.append(img)
        else:
            resizedImages.append(blank)
    barCode = cv2.resize(barCode, (1600,100), interpolation=cv2.INTER_CUBIC)
    ...
    for row in range(0,rows):
        ...
        for column in range(0,columns):
            columnArray.append(resizedImages[column + (row * columns)])
        rowArray.append(cv2.hconcat(columnArray))
    rowArray.append(barCode)
    return cv2.vconcat(rowArray)
...
blankImg = cv2.imread("tmp/" + str(count) + ".png")
...
for index in range(1,count):
    #print("\033c")
    if index % codesPerPage == 1:
        imgArr = []
        for i in range(index, index + codesPerPage):
            if i < count:
                imgArr.append(cv2.imread("tmp/" + str(i) + ".png"))
        print("loaded " + str(newCount) + " of " + str(count / codesPerPage))
        newCount += 1
        cv2.imwrite("tmp/new_" + str(newCount) + ".png",joinImages(imgArr,barcode,blankImg))
        image = imageio.imread("tmp/new_" + str(newCount) + ".png")
        images.append(image)



#print("\033c")
print("generating data.gif - " + str(newCount))
